...
def decode_nested_txt_line(line_content):
    ...
    if len(parts) >= 3:
        ...
        try:
            ...
            else: error = "Không tìm thấy nội dung lồng nhau."
        except Exception as e: error = f"Lỗi giải mã: {e}"
    else: error = "Định dạng dòng TXT không hợp lệ."
...
def infer_case_type_from_filename(filename: str) -> str:
    ...
    for tok in tokens:
        if tok.upper() == "CASENUMBERFILE": return "CASENUMBERFILE"
    try:
...
def run_md_moi_check(directory_path):
    ...
    if not xml_files: return "Không tìm thấy tệp .xml nào trong thư mục."
    ...
    for xml_file in xml_files:
        ...
        if not case_type_from_name:
            log.append(f"  ❌ Lỗi: Không thể lấy Case Type từ '{xml_file}'.")
            continue
        ...
        try:
            ...
            for lead in root.findall('.//ns:Lead', namespace):
                ...
                if lead_id and case_key: case_keys_from_xml[lead_id] = case_key
        ...
        try:
            with open(txt_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if uuid: uuid_to_html[uuid] = (html, error)
        except Exception as e:
        ...
        for lead_id, case_key_raw in case_keys_from_xml.items():
            ...
            if "DATA NOT FOUND" in html_content:
                ...
                if not fn_html or fn_html.group(1).strip() != first_name_xml: lead_errors.append("First Name")
                if not ln_html or ln_html.group(1).strip() != last_name_xml: lead_errors.append("Last Name")
                try:
