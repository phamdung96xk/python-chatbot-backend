    ...
    if not details_row:
        ...
        if not file_date_cells:
            ...
            try:
                ...
                date_format = "%m/%d/%Y"
                xml_date_from_obj = datetime.strptime(normalize_date_str(xml_date_from_str), date_format)
                xml_date_to_obj = datetime.strptime(normalize_date_str(xml_date_to_str), date_format)
                html_file_date_obj = datetime.strptime(normalize_date_str(html_file_date_str), date_format)
                
                if not (xml_date_from_obj <= html_file_date_obj <= xml_date_to_obj):
...
def run_civitek_new_check(directory_path):
    ...
    content_files = list(dir_path.glob("*_content.txt"))
    ...
    for content_file_path in content_files:
        ...
        xml_file_path = dir_path / f"{base_name}.xml"
